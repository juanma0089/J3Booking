...
function pintarTabla(users) {
    ...
    for (let i = 0; i < users.length; i++) {
        ...
        `<button type="button" user="${user.id}" class="align-self-lg-center p-0 m-0 bi bi-pen text-warning bg-transparent border-0 edit-btn"></button>` +
        `<button type="button" user="${user.id}" class="align-self-lg-center p-0 m-0 bi bi-x-lg text-danger bg-transparent border-0 delete-btn"></button></div></div>`;
