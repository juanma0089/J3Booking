...
function ajaxQuery(action) {
    ...
    if (match) {
        ...
        var eventinput = $("input[name='rrpp']").val() ?? match[1];
        var rrppinput = $("input[name='rrpp']").val() ?? 'all';
    } else {
        urlevent = '/books'
        var eventinput = $("input[name='rrpp']").val() ?? 'none';
        var rrppinput = $("input[name='rrpp']").val() ?? 'all';
    }
...
function pintarTabla(books) {
    ...
    for (let i = 0; i < books.length; i++) {
        ...
        if (book.type == 'pista') {
            html += `<p class="align-self-lg-center text-center text-white bi bi-bookmark p-0 m-0">${book.type}</p></div>`;
        }
        else {
            html += `<p class="align-self-lg-center text-center text-info bi bi-bookmark-star p-0 m-0">VIP</p></div>`;
        }
        if (book.status == 'waiting') {
            html += `<div class="align-self-center px-lg-2 px-sm-0 px-md-1 flex-fill col-4 col-lg-3 d-flex justify-content-center"><p class="align-self-lg-center text-center text-warning bi bi-hourglass-split p-0 m-0">${book.status}</p></div>`;
        }

        else if (book.status == 'accepted') {
            html += `<div class="align-self-center px-lg-2 px-sm-0 px-md-1 flex-fill col-4 col-lg-3 d-flex justify-content-center"><p class="align-self-lg-center text-center text-success bi bi-check p-0 m-0">${book.status}</p></div>`;
        }
        else {
            html += `<div class="align-self-center px-lg-2 px-sm-0 px-md-1 flex-fill col-4 col-lg-3 d-flex justify-content-center"><p class="align-self-lg-center text-center text-danger bi bi-x p-0 m-0">${book.status}</p></div>`;
        }
        ...
        `<div class="accordion-item">`+
            ...
            `<p class="fw-lighter border rounded-5 fs-6 mb-0 text-white p-1 bi bi-eye px-2" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseOne_${book.id}" aria-expanded="true" aria-controls="flush-collapseOne" data-target="name">`+
            ` Botellas</p>`+
            `</h2>`+
            `</div>`+
            `</div></div></div>` +
