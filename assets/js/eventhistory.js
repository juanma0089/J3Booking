...
function pintarTabla(events) {
    ...
    for (let i = 0; i < events.length; i++) {
        const event = events[i];
            ...
            `<img src="./public/${event.image}" alt="Evento ${event.id}" class="img-fluid img-event">` +
            '</div>' +
            ...
            `<a data-evento-id="${event.id}" class="bg-transparent border-0 align-self-lg-center p-3 text-dark d-flex justify-content-evenly">`+
