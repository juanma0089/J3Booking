...
function printTables(tables) {
    ...
    tables.forEach(table => {
        if (table.type == 'escenario') {
            ...
            html += htmlTypeTable(table.id, table.type, table.idbook, table.statusbook, table.nummesa);
        }

        if (table.type == 'altaescenario') {
            if (firstTableRow < 2) {
                html +=
                    '</div><hr class="col-12 text-white">' +
                    '<div class="row d-flex justify-content-center align-content-center mt-4">' +
                    '<h1 class="text-center">Mesas altas VIP Escenario</h1>' +
                    '<p class="text-center">Botellas min (' + minVipEsc + ')</p>';
...
function htmlTypeTable(id, type, has_booking = null, status, nummesa = null) {

    switch (type) {

        case 'altaescenario':
        case 'mesaalta':
            return '<div class="col d-flex flex-column align-items-center p-3">' +
                `<span class="mt-2">-${nummesa}-</span>` +
        ...
        default:
            return '<div class="col d-flex flex-column align-items-center p-0">' +
                `<span class="mt-2">-${nummesa}-</span>` +
